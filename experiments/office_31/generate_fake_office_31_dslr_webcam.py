        ...
        for i, (data, data_tgt) in enumerate(zip(dataloader, cycle(dataloader_tgt)), 0):
            ...
            if (iters % 500 == 0) or ((epoch == num_epochs-1) and (i == len(dataloader)-1)):
                with torch.no_grad():
                    ...
                    try:
                        shutil.rmtree('generated_images//office_31_dslr_to_webcam//'+str(category) + '//')
                    except:
                        pass
                    os.makedirs('generated_images//office_31_dslr_to_webcam//'+str(category) + '//')
                    save_individual_images('generated_images//office_31_dslr_to_webcam//'+str(category) + '//', fake)
